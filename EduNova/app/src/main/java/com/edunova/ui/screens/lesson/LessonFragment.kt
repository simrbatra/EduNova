    ...
    private fun setupObservers() {
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.lessons.observe(viewLifecycleOwner) { lessons ->
                val filteredSubject = viewModel.filteredSubject.value
                val filteredLessons = if (filteredSubject != null) {
                    lessons.filter { it.subject == filteredSubject }
                } else {
                    lessons
                }
                lessonAdapter.submitList(filteredLessons)
            }
